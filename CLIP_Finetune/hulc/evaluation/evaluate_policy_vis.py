...
def evaluate_policy(model, env, lang_embeddings, args):
    ...
    for initial_state, eval_sequence in eval_sequences:
        result, imgs_all = evaluate_sequence(
            env, model, task_oracle, initial_state, eval_sequence, lang_embeddings, val_annotations, args, plans, imgs_all, seq_num
        )
        seq_num = seq_num + 1
    ...
    video = cv2.VideoWriter("hulc/vis_D_no_reset_128.avi", cv2.VideoWriter_fourcc(*"XVID"), 15, (500, 500))
    for img in imgs_all:
        video.write(img)
    video.release()
...
def evaluate_sequence(
    env, model, task_checker, initial_state, eval_sequence, lang_embeddings, val_annotations, args, plans, imgs_all, seq_num
):
    ...
    for subtask in eval_sequence:
        success, imgs_all = rollout(env, model, task_checker, args, subtask, lang_embeddings, val_annotations, plans, imgs_all, seq_num)
        # if success:
        #     success_counter += 1
        # else:
        #     return success_counter, imgs_all
    return success_counter, imgs_all


def rollout(env, model, task_oracle, args, subtask, lang_embeddings, val_annotations, plans, imgs_all, seq_num):
    if args.debug:
        print(f"{subtask} ", end="")
        time.sleep(0.5)
    ...
    img = cv2.putText(
        imgRGB, f"{seq_num}. {lang_annotation}", (100, 20), font, color=(0, 0, 0), fontScale=0.5, thickness=1
    )[:, :, ::-1]
    imgs_all.append(img)
    # for _ in range(25):
    #     imgs_all.append(img)
