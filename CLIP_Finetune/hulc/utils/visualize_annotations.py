...
def generate_all_seq_gifs(data, dataset):
    ...
    for i, idx in enumerate(tqdm(data["info"]["indx"][:20])):
        ...
        start = int(np.where(dataset.episode_lookup == idx[0])[0][0])
        print('start:', start)
...
def plot_and_save_gifs(imgs):
    ...
    video = cv2.VideoWriter("hulc/summary_lang_anns_clip_temp_8.avi", cv2.VideoWriter_fourcc(*"XVID"), 15, (500, 500))
    for img in imgs:
        video.write(img)
    video.release()
