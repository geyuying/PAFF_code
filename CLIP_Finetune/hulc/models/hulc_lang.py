...
from hulc.models.encoders.clip_vision_encoder import VisionClip
...
class Hulc(pl.LightningModule):
    ...
    def __init__(
        ...
        self.ann_embeddings = self.clip_text_encoder(ann_all)
    ...
    def training_step(self, batch: Dict[str, Dict], batch_idx: int) -> torch.Tensor:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            if "lang" in self.modality_scope:
                ...
                sim = self.clip_vision_encoder(visual_input, self.ann_embeddings)
                ...
                for j in range(perceptual_emb.shape[0]):
                    ann_index = sim.argmax(-1)[j]
                    score = sim[j][ann_index]
                    ann  = self.ann_all[ann_index]
                    save_name = os.path.join('hulc/vis_4frame_clip_ft_epoch7', ann + '.jpg')
                    if os.path.exists(save_name):
                        continue

                    for i in range(4):
                        idx = sample_idx[i]
                        image =  dataset_batch["rgb_obs"]["rgb_static"][j, idx].cpu().numpy()
