...
class Hulc(pl.LightningModule):
    ...
    def training_step(self, batch: Dict[str, Dict], batch_idx: int) -> torch.Tensor:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            anns = dataset_batch["raw"]
            images_batch = dataset_batch["rgb_obs"]["rgb_static"].cpu().numpy()
            bz = dataset_batch["actions"].shape[0]
            imgs_all = []
            font = cv2.FONT_HERSHEY_SIMPLEX
            for i in range(bz):
                ann = anns[i]
                images = images_batch[i]
                for j in range(images.shape[0]):
                    ...
                    img = cv2.putText(
                        imgRGB, f"{i}. {ann}", (100, 20), font, color=(0, 0, 0), fontScale=0.5, thickness=1
                    )[:, :, ::-1]
                    imgs_all.append(img)

            video = cv2.VideoWriter("hulc/vis_video/" + str(self.iter)+"_vis_gen_training.avi", cv2.VideoWriter_fourcc(*"XVID"), 15,
                                    (500, 500))
            for img in imgs_all:
                video.write(img)
            video.release()
