...
def rollout(env, model, task_oracle, args, subtask, ann_dict, plans, seq_num, task_num):
    ...
    save_dir = 'hulc/record_D_256_random_ann_reset_rebot_per_task_reset_all_per_50_epoch8_vis/'
    ...
    for step in range(args.ep_len):
        ...
        rgb_static = cv2.cvtColor(rgb_static, cv2.COLOR_RGB2BGR)
        if step % 8 == 0:
            save_name = save_dir + subtask+ '_' + 'sequence_' + str(seq_num).zfill(5) + '_' + str(task_num) + '_' + str(step).zfill(4) + '.jpg'
            print(save_name)
            cv2.imwrite(save_name, rgb_static)
