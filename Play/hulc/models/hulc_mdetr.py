...
from hulc.models.perceptual_encoders.mdetr_cross_encoders import Mdetr
...
class Hulc(pl.LightningModule):
    ...
    def __init__(
        ...
        if lang_discriminator:
            ...
            self.proj_vis_lang = hydra.utils.instantiate(clip_proj)
        self.mdetr_cross_encoder = hydra.utils.instantiate(fuse_visual_lang)
    ...
    def lmp_train(
        ...
        """
        # ------------Plan Proposal------------ #
        pp_state = self.plan_proposal(perceptual_emb[:, 0, 256:], latent_goal)
        ...
        action_loss = self.action_decoder.loss(
            sampled_plan, perceptual_emb, latent_goal[:, 256:], train_acts, robot_obs
        )  # type:  ignore
    ...
    def lmp_val(
        ...
        """
        # ------------Plan Proposal------------ #
        pp_state = self.plan_proposal(perceptual_emb[:, 0, 256:], latent_goal)  # (batch, 256) each
        pp_dist = self.dist.get_dist(pp_state)
        ...
        action_loss_pp, sample_act_pp = self.action_decoder.loss_and_act(  # type:  ignore
            sampled_plan_pp, perceptual_emb, latent_goal[:, 256:], actions, robot_obs
        )
        ...
        action_loss_pr, sample_act_pr = self.action_decoder.loss_and_act(  # type:  ignore
            sampled_plan_pr, perceptual_emb, latent_goal[:, 256:], actions, robot_obs
        )
    ...
    def training_step(self, batch: Dict[str, Dict], batch_idx: int) -> torch.Tensor:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            perceptual_emb, src, pos_embed, mask = self.perceptual_encoder(
            ...
            if "lang" in self.modality_scope:
                #print(dataset_batch["lang"].shape, dataset_batch["raw"])
                latent_goal = self.language_goal(dataset_batch["lang"])
                static_lang_fused = self.mdetr_cross_encoder(dataset_batch["lang"], dataset_batch["raw"], src, pos_embed, mask)
            else:
                latent_goal = self.visual_goal(perceptual_emb[:, -1])
                static_lang_fused = perceptual_emb[:, 0, :256]

            latent_goal_cat = torch.cat((static_lang_fused, latent_goal), 1)
            kl, act_loss, mod_loss, pp_dist, pr_dist, seq_feat = self.lmp_train(
                perceptual_emb, latent_goal_cat, dataset_batch["actions"], dataset_batch["state_info"]["robot_obs"]
            )
            if "lang" in self.modality_scope:
    ...
    def step(self, obs, goal, raw):
        ...
        if self.rollout_step_counter % self.replan_freq == 0:
            if "lang" in goal:
                self.plan, self.latent_goal = self.get_pp_plan_lang(obs, goal, raw)
            else:
    ...
    def predict_with_plan(
        ...
        """
        with torch.no_grad():
            perceptual_emb, src, pos_embed, mask = self.perceptual_encoder(obs["rgb_obs"], obs["depth_obs"], obs["robot_obs"])
    ...
    def get_pp_plan_lang(self, obs: dict, goal: dict, raw: list) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Use plan proposal network to sample new plan using a visual goal embedding.
        ...
        with torch.no_grad():
            ...
            static_lang_fused = self.mdetr_cross_encoder(goal["lang"], raw, src, pos_embed, mask)
            latent_goal_cat = torch.cat((static_lang_fused, latent_goal), 1)

            # ------------Plan Proposal------------ #
            pp_state = self.plan_proposal(perceptual_emb[:, 0, 256:], latent_goal_cat)
            pp_dist = self.dist.get_dist(pp_state)
            sampled_plan = self.dist.sample_latent_plan(pp_dist)
        self.action_decoder.clear_hidden_state()
        return sampled_plan, latent_goal
