...
class Mdetr(nn.Module):
    ...
    def forward(self, lang, raw, src, pos_embed, mask):
        ...
        with torch.no_grad():
            ...
            lang_memory = img_memory[num:].transpose(0, 1)
            text_attention_mask = tokenized.attention_mask.float()
            lang_memory = lang_memory * text_attention_mask.unsqueeze(-1).repeat(1, 1, lang_memory.shape[-1])
            lang_memory_mean = torch.sum(lang_memory, 1) / torch.sum(text_attention_mask, 1).unsqueeze(-1).repeat(1, lang_memory.shape[-1])
            lang_cat = torch.cat((lang, lang_memory_mean), 1)
            #print(lang_memory.shape, text_attention_mask.shape, lang_memory_mean.shape, lang.shape, lang_cat.shape)
        return lang_cat
