...
class Annotator(Callback):
    def __init__(self, cfg):
        ...
        mdetr_ann_file = 'hulc/dataset/task_ABC_D/training/mdetr/auto_lang_ann.npy'
        mdetr_ann = np.load(mdetr_ann_file, allow_pickle=True).item()
        self.train_task = mdetr_ann['language']['task']
        self.train_ann = mdetr_ann['language']['ann']
        self.train_indx_all = np.array(mdetr_ann['info']['indx'])[:,0]
    ...
    def annotate(self, episode, dataset, collected_data, global_task_counter, num_samples):
        ...
        seq_idx = dataset.episode_lookup[idx.cpu()]
        print(seq_idx)
        if 744905 not in seq_idx:
        ...
        for i in range(batch_size):
            ...
            print(start_idx, dataset.episode_lookup[start_idx], task)
