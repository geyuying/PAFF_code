...
class DiskDataset(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        try:
            print("trying to load lang data from: ", self.abs_datasets_dir / self.lang_folder / "auto_lang_ann.npy")
            lang_data = np.load(self.abs_datasets_dir / self.lang_folder / "auto_lang_ann.npy", allow_pickle=True).item()
        except Exception:
            print("Exception, trying to load lang data from: ", self.abs_datasets_dir / "auto_lang_ann.npy")
            lang_data = np.load(self.abs_datasets_dir / "auto_lang_ann.npy", allow_pickle=True).item()

        self.ep_start_end_ids = lang_data["info"]["indx"]  # each of them are 64
        self.lang_ann = lang_data["language"]["emb"]  # length total number of annotations
        self.lang_raw = lang_data["language"]["ann"]
        self.lang_task = lang_data["language"]["task"]
        self.num_frames = 8
    ...
    def _load_episode(self, idx: int) -> Dict[str, np.ndarray]:
        ...
        """
        start_idx = self.ep_start_end_ids[idx][0]
        end_idx = self.ep_start_end_ids[idx][1]
        ...
        episodes = [self.load_file(self._get_episode_name(file_idx)) for file_idx in frame_idxs]
        episode = {key: np.stack([ep[key] for ep in episodes]) for key in keys}
        if self.with_lang:
            episode["language"] = self.lang_ann[idx][0]  # TODO check  [0]
            episode["raw"] = self.lang_raw[idx]
            episode["task"] = self.lang_task[idx]
        return episode
    ...
    def _build_file_indices_lang(self, abs_datasets_dir: Path) -> Tuple[np.ndarray, List, np.ndarray]:
        ...
        return np.array(episode_lookup), lang_lookup, lang_ann, lang_raw, lang_task
