...
def process_language(episode: Dict[str, np.ndarray], transforms: Dict, with_lang: bool, ann_dict: Dict) -> Dict[str, torch.Tensor]:
    seq_lang = {"lang": torch.empty(0), "raw": [], "raw_label": torch.empty(0)}
    if with_lang:
        lang = torch.from_numpy(episode["language"]).float()
        raw = episode["raw"]
        #raw = "A white robot arm " + episode["raw"]
        #label = torch.from_numpy(np.array([ann_dict[raw]]))
        #task = episode["task"]
        #raw = ann_dict[task][0]
        #label = torch.from_numpy(np.array([ann_dict[task][1]]))
        #label = torch.from_numpy(np.array([ann_dict[task][0]]))
        if "language" in transforms:
            lang = transforms["language"](lang)
        seq_lang["lang"] = lang
        seq_lang["raw"] = raw
        #seq_lang["raw_label"] = label
    return seq_lang
