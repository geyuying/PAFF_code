...
class Hulc(pl.LightningModule):
    ...
    def __init__(
        ...
        # intervals = np.linspace(start=0, stop=32, num=4 + 1).astype(int)
        # ranges = []
        # for idx, interv in enumerate(intervals[:-1]):
        #     ranges.append((interv, intervals[idx + 1] - 1))
        # self.ranges = ranges

        ann_file = 'hulc/dataset/task_D_D/training/mdetr/auto_lang_ann.npy'
        ann_content = np.load(ann_file, allow_pickle=True).item()
        ann_all = []
        for ann in ann_content['language']['ann']:
            ann = "A white robot arm " + ann
            if ann not in ann_all:
                ann_all.append(ann)
        self.ann_embeddings = self.clip_text_encoder(ann_all).cpu()
        self.ann_all = ann_all
    ...
    def training_step(self, batch: Dict[str, Dict], batch_idx: int) -> torch.Tensor:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            #sample_idx = [random.choice(range(x[0], x[1])) for x in self.ranges]
            #visual_input = dataset_batch["rgb_obs"]["rgb_static"][:, sample_idx]
            visual_input = dataset_batch["rgb_obs"]["rgb_static"]
            lang_input = dataset_batch['raw']

            visual_features = self.clip_vision_encoder(visual_input)
            #print(visual_input.shape, visual_features.shape)
            #lang_features = self.clip_text_encoder(self.ann_all)
            lang_features = self.ann_embeddings.to(self.device)


            #print(visual_features.shape, lang_features.shape, lang_labels.shape)
            #print(dataset_batch["raw"][0], self.ann_all[lang_labels[0]])
            ...
            bz = dataset_batch["actions"].shape[0]
            total_bs += dataset_batch["actions"].shape[0]

            sim = logits_per_image.detach().cpu().numpy()
            #print(visual_input.shape, visual_features.shape, sim.shape)
            ...
            for j in range(bz):
                ...
                if score > 5.0:
                    self.num = self.num + 1
                    idx = dataset_batch["idx"][j]
                    ann = self.ann_all[ann_index].replace("A white robot arm ", "")
                    f = open('hulc/lang_from_clip_8_frames_temp.txt', 'a')
                    f.write(str(idx))
                    f.write(' ')
                    f.write(ann)
                    f.write('\n')
                    ...
                    ann = self.ann_all[ann_index]
                    save_name = os.path.join('hulc/vis_8frame_clip_temp_window64_score', ann + '.jpg')
                    if os.path.exists(save_name):
                        continue
                #
                    for i in range(8):
                        image = dataset_batch["rgb_obs"]["rgb_static"][j, i].cpu().numpy()
    ...
    def clip_loss(self, image_features, lang_features):
        ...
        #print(logits_per_image.shape, labels.shape)
        #logits_per_text = logits_per_image.t()

        # symmetric loss function
        labels = torch.arange(logits_per_image.shape[0], device=text_features.device)
        loss = cross_entropy(logits_per_image, labels) * 0.0
    ...
    def validation_step(self, batch: Dict[str, Dict], batch_idx: int) -> Dict[str, torch.Tensor]:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            #sample_idx = [(x[0] + x[1]) // 2 for x in self.ranges]
            #visual_input = dataset_batch["rgb_obs"]["rgb_static"][:, sample_idx]
            visual_input = dataset_batch["rgb_obs"]["rgb_static"]
            lang_input = dataset_batch['raw']
