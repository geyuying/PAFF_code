...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention_frames(self, x: torch.Tensor):
        ...
        x_ = x_.reshape(-1, 36, x_.shape[-1])
        n_f = int(x_.shape[0] / bz)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        #print(x.shape)
        if x.shape[0] == 77 or x.shape[0] == 37 or x.shape[0] == 5:
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.temporal_embedding = nn.Parameter(torch.zeros(self.num_frames, width))
        ...
        self.fc = nn.Linear(in_features=768, out_features=34)
    ...
    def forward(self, x: torch.Tensor):
        ...
        #tile_pos_embed = self.positional_embedding[1:37, :].repeat(n_frames, 1)
        tile_pos_embed = self.positional_embedding[1:(h*w+1), :].repeat(n_frames, 1)
        #tile_temporal_embed = self.temporal_embedding.repeat_interleave(36, 0)
        tile_temporal_embed = self.temporal_embedding[0:n_frames].repeat_interleave(h*w, 0)
        #print(tile_pos_embed.shape, tile_temporal_embed.shape)
        total_pos_embed = tile_pos_embed + tile_temporal_embed
        #total_pos_embed = tile_pos_embed
        #print(cls_embed.shape, tile_pos_embed.shape, tile_temporal_embed.shape, total_pos_embed.shape)
