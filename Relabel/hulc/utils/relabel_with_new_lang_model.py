...
def main(cfg: DictConfig) -> None:
    ...
    ann_data = data["language"]["ann"]
    bz = 32
    ann_num = len(ann_data)
    num = int(ann_num / bz)
    for i in range(0,num):
        ann_input = data["language"]["ann"][i*bz:(i+1)*bz]
        output =  model(ann_input).cpu().numpy()
        if i == 0:
            output_all = output
        else:
            output_all = np.concatenate((output_all, output), 0)
    ann_input = data["language"]["ann"][(i+1)*bz:]
    output =  model(ann_input).cpu().numpy()
    output_all = np.concatenate((output_all, output), 0)
    data["language"]["emb"] = output_all
    print("Saving data")
    save_path = path.parent / ".." / cfg.name_folder
    save_path.mkdir(exist_ok=True)
    np.save(save_path / "auto_lang_ann.npy", data)
    print(len(data["language"]["ann"]), data["language"]["emb"].shape)
