...
def generate_all_seq_gifs(data, dataset):
    imgs = []
    indx_all = np.arange(20)
    np.random.shuffle(indx_all)
    np.random.shuffle(indx_all)
    # fig = plt.figure()
    #for i, record_idx in enumerate(tqdm(data["info"]["indx"][:20])):
    for i in range(20):
        indx = indx_all[i]
        record_idx = data["info"]["indx"][indx]

        # seq_id = int(record_idx / 30)
        # task_id = int((record_idx % 30) / 6)
        # step_id = int((record_idx % 30) % 6)
        #
        ...
        seq_id = int(record_idx / 35)
        task_id = int((record_idx % 35) / 7)
        step_id = int((record_idx % 35) % 7)
        ...
        start1 = np.where(dataset.episode_lookup == start_idx)[0]
        start2 = np.where(dataset.episode_lookup_seq == seq_id)[0]
        start3 = np.where(dataset.episode_lookup_task == task_id)[0]

        start1 = np.intersect1d(start1, start2)
        start = int(np.intersect1d(start1, start3)[0])
        print(start, indx)
        #start = dataset.episode_lookup.index(idx[0])
        seq_img = dataset[start]["rgb_obs"]["rgb_static"].numpy()
        # if 'lift' in data['language']['task'][i]:
        imgs = generate_single_seq_gif(seq_img, seq_length, imgs, start, indx, data)
    return imgs
...
def load_data(cfg):
    ...
    data_module.setup()
    dataset = data_module.train_dataloader()["lang"].dataset
...
def plot_and_save_gifs(imgs):
    ...
    video = cv2.VideoWriter("hulc/summary_lang_anns_clip_temp_8_new.avi", cv2.VideoWriter_fourcc(*"XVID"), 15, (500, 500))
    for img in imgs:
        video.write(img)
    video.release()
