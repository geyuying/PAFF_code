...
class Hulc(pl.LightningModule):
    ...
    def training_step(self, batch: Dict[str, Dict], batch_idx: int) -> torch.Tensor:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            if "lang" in self.modality_scope:
                lang_cat = self.mdetr_cross_encoder(dataset_batch["lang"], dataset_batch["raw"], src, pos_embed, mask)
                latent_goal = self.language_goal(lang_cat)
            else:
            ...
            if "lang" in self.modality_scope:
                if not torch.any(dataset_batch["use_for_aux_lang_loss"]):
                    ...
                    if self.img_lang_matching_clip:
                        lang_clip_loss += self.clip_loss(seq_feat, dataset_batch["lang"], dataset_batch["use_for_aux_lang_loss"])
    ...
    def validation_step(self, batch: Dict[str, Dict], batch_idx: int) -> Dict[str, torch.Tensor]:  # type: ignore
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            if "lang" in self.modality_scope:
                ...
                if self.img_lang_matching_clip:
                    val_pred_clip_loss = self.clip_loss(seq_feat, dataset_batch["lang"], dataset_batch["use_for_aux_lang_loss"])
    ...
    def get_pp_plan_lang(self, obs: dict, goal: dict, raw: list) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            perceptual_emb, src, pos_embed, mask = self.perceptual_encoder(obs["rgb_obs"], obs["depth_obs"], obs["robot_obs"])
            lang_cat = self.mdetr_cross_encoder(goal["lang"], raw, src, pos_embed, mask)
            latent_goal = self.language_goal(lang_cat)
    ...
    def on_validation_epoch_start(self) -> None:
        ...
        if self.img_lang_matching_clip:
            ...
            self.encoded_lang_train = self.train_lang_emb
            self.encoded_lang_val = self.val_lang_emb
    ...
    def clip_inference(self, obs: dict, goal: dict) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        with torch.no_grad():
            perceptual_emb, _, _, _ = self.perceptual_encoder(obs["rgb_obs"], obs["depth_obs"], obs["robot_obs"])
            encoded_lang = goal["lang"]
