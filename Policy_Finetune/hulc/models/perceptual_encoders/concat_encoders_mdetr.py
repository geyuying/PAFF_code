...
class ConcatEncoders(nn.Module):
    def __init__(
        ...
        #print(self.rgb_static_encoder, self.rgb_gripper_encoder)
    ...
    def forward(
        ...
        #print(rgb_static.shape, rgb_gripper.shape)

        b, s, c, h, w = rgb_static.shape
        rgb_static = rgb_static.reshape(-1, c, h, w)  # (batch_size * sequence_length, 3, 200, 200)
        # ------------ Vision Network ------------ #
        encoded_imgs, src, pos_embed, mask = self.rgb_static_encoder(rgb_static)  # (batch*seq_len, 64)
        src = src.reshape(src.shape[0], b, s, -1)[:,:,0]
        pos_embed = pos_embed.reshape(pos_embed.shape[0], b, s, -1)[:,:,0]
        mask = mask.reshape(b, s, -1)[:,0]
        #print('static', encoded_imgs.shape)
        ...
        if rgb_gripper is not None:
            ...
            encoded_imgs_gripper, _, _, _ = self.rgb_gripper_encoder(rgb_gripper)  # (batch*seq_len, 64)
            #print('gripper', encoded_imgs_gripper.shape)
        ...
        return perceptual_emb, src, pos_embed, mask
